    ...
    var body: some View {
        ZStack { Theme.surface1.ignoresSafeArea()
            VStack(spacing: 0) {
                ...
                ScrollView {
                    switch selectedTab {
                    case 0: BattingCard(vm: vm)
                    case 1: BowlingCard(vm: vm)
                    default: FallOfWicketsCard(vm: vm)
                    }
                }
            }
        }
...
struct BattingCard: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            ForEach(vm.innings.batterStats) { b in
                HStack {
                    VStack(alignment: .leading, spacing: 1) {
                        HStack(spacing: 3) {
            ...
            HStack {
                ...
                Spacer()
                Text("\(vm.innings.extras) (wd \(vm.innings.wides), nb \(vm.innings.noBalls), b \(vm.innings.byes), lb \(vm.innings.legByes))")
            ...
            HStack {
                ...
                Spacer()
                Text("\(vm.innings.runs)/\(vm.innings.wickets) (\(vm.innings.oversDisplay) ov)")
...
struct FallOfWicketsCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            if vm.innings.fallOfWickets.isEmpty {
                Text("No wickets fallen yet").font(.system(size: 13)).foregroundColor(Theme.text3)
                    .frame(maxWidth: .infinity).padding(.vertical, 24)
            }
            ForEach(0..<vm.innings.fallOfWickets.count, id: \.self) { i in
                let (r, w, name) = vm.innings.fallOfWickets[i]
                HStack {
                ...
                .padding(.horizontal, 16).padding(.vertical, 8)
                if i < vm.innings.fallOfWickets.count - 1 { Divider().background(Theme.border).padding(.horizontal, 8) }
            }
        }.padding(.top, 8)
    }
}
    ...
    var body: some View {
        ZStack { Theme.surface1.ignoresSafeArea()
            VStack(spacing: 24) {
                ...
                GreenButton("Start 2nd Innings →") { vm.beginInnings2() }.padding(.horizontal, 20)
                Spacer()
            }
        }
    }
