...
struct RunInputPanel: View {
    ...
    let bottomCols = Array(repeating: GridItem(.flexible(), spacing: 6), count: 4)
    ...
    var body: some View {
        VStack(spacing: 8) {
            ...
            LazyVGrid(columns: bottomCols, spacing: 6) {
                ...
                RunButton(label: "UNDO",
                          color: vm.canUndo ? Theme.red : Theme.text3,
                          bg: (vm.canUndo ? Theme.red : Theme.text3).opacity(0.1),
                          border: (vm.canUndo ? Theme.red : Theme.text3).opacity(0.4),
                          fontSize: 13) {
                    withAnimation { vm.undoLastBall() }
                }
                .disabled(!vm.canUndo)
